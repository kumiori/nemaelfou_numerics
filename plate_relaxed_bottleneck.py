...
class RelaxedExperiment(Experiment):
	...
	def add_userargs(self):
		self.parser.add_argument("--Lx", type=float, default=1.)
		self.parser.add_argument("--Ly", type=float, default=.5)
	...
	def create_mesh(self, parameters):
		...
		if os.path.isfile(meshfile):
			...
			nel = int(parameters['geometry']['Lx']/parameters['geometry']['meshsize'])

		RectangleMesh(Point(0., 0.), Point(parameters['geometry']['Lx'], parameters['geometry']['Ly']), 100, 50)
	...
	def define_boundary_conditions(self):
		...
		ut = Expression([ 't', 0.], t=0, degree=0)
		...
		bc_free = []
		traction = 'on_boundary and near(x[0], {})'.format(geometry['Lx'])
		clamped = 'on_boundary and near(x[0], {})'.format(0)
		bc_vert = [DirichletBC(V.sub(2), Constant(0.), 'on_boundary')]
		bc_horiz = [DirichletBC(V.sub(1), Constant([0., 0.]), 'on_boundary')]
		bc_traction = [DirichletBC(V.sub(1), Constant([0., 0.]), 'left'),
			DirichletBC(V.sub(2), Constant(0.), 'left'),
			DirichletBC(V.sub(2), Constant(0.), 'trax'),
			DirichletBC(V.sub(1), ut, 'left'),
			]
...
data = []
problem = RelaxedExperiment(template='', name='bottleneck', p='zero')
problem.counter = 0
